...
export class FullComponent implements OnInit {
    ...
    private loadMenuFromStorage(): void {
        ...
        try {
            const rawMenu: any[] = JSON.parse(storedMenu);

            this.navItems = rawMenu.map((item) => this.mapMenuItemToNavItem(item));
        } catch (e) {
            console.error('Erro ao parsear menu do localStorage, usando padrão', e);
            this.navItems = navItems;
        }
    }

    private mapMenuItemToNavItem(item: any): NavItem {
        const route =
            item.route && item.route !== ''
                ? '/' + String(item.route).replace(/^\/+/, '')
                : undefined;

        return {
            navCap: item.navCap || undefined,
            displayName: item.displayName || undefined,
            iconName: item.iconName || undefined,
            route: route,
            // se seu NavItem tiver mais campos (children, disabled, external etc),
            // você pode mapear aqui também:
            // disabled: item.disabled ?? false,
            // external: item.external ?? false,
            // chip: item.chip ?? false,
            // chipClass: item.chipClass ?? undefined,
            // chipContent: item.chipContent ?? undefined,
        } as NavItem;
    }
}
